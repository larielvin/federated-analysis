...
import subprocess
import tarfile, tempfile, secrets, hashlib, base64, json
...
# ---- Optional import (for CBOR parsing if needed) ----
try:
    import cbor2
except Exception:
    cbor2 = None
...
# ---- paths & setup ----
REPO_DIR = Path.cwd()
DEPLOYMENT_PATH = REPO_DIR / "deployment_files" / "model"
FHE_KEYS        = REPO_DIR / ".fhe_keys"
EVAL_KEYS       = REPO_DIR / "evaluation_keys"
OUTPUT_FILES    = REPO_DIR / "output"
POLICY_PATH = REPO_DIR / "age_files" / "recipients" / "recipients.json"
...
def archive_fhe_keys():
    ...
    if FHE_KEYS.exists():
        shutil.rmtree(FHE_KEYS, ignore_errors=True)
...
def archive_evaluation_keys():
    ...
    if EVAL_KEYS.exists():
        shutil.rmtree(EVAL_KEYS, ignore_errors=True)
...
# ---------------- age wrapping (mandatory) ----------------
@report_errors("run_age_encrypt")
def run_age_encrypt(data: bytes, age_pubkey: str, out_path: Path):
    try:
        subprocess.run(
            ["age", "-r", age_pubkey, "-o", str(out_path)],
            input=data,
            check=True
        )
        return out_path, "age"
    except FileNotFoundError:
        msg = "age binary not found; aborting (no raw-share fallback)."
        send_log(msg, "ERROR")
        raise RuntimeError(msg)
    except subprocess.CalledProcessError as e:
        msg = f"age failed (rc={e.returncode}); aborting (no raw-share fallback)."
        send_log(msg, "ERROR")
        raise
...
def write_pubkey(pubkey_raw: bytes, out_dir: Path) -> Path:
    ...
    out_path = out_dir / "runtime_pubkey.ed25519.b64"
...
def encrypt_locally(
    ...
    # 5) Runtime signing key (local)
    sk, pub_raw = generate_signing_key_in_memory()
    pub_path = write_pubkey(pub_raw, OUTPUT_FILES)

    attestation_cbor = get_attestation_doc(b"ed25519-pubkey:" + pub_raw)  # empty
    pcr0_hex = parse_pcr0_from_attestation_cbor(attestation_cbor) if attestation_cbor else ""
    att_sha = ""
    ...
    receipt = {
        ...
        "attestation_cbor_sha256": att_sha,
        "pcr0_image_sha384_hex": pcr0_hex,
        "mode": "local-logs-only"
    }
    ...
    send_log(f"Runtime public key: {pub_path}")
...
if __name__ == "__main__":
    send_log("app boot (log-only mode)")

    try:
        # Enforce recipients hash only if set
        if os.getenv("RECIPIENTS_JSON_SHA256"):
            h = hashlib.sha256(Path("./age_files/recipients/recipients.json").read_bytes()).hexdigest()
        ...
        encrypt_locally(Path(fhekeys_archive_path), Path(evaluation_archive_path), client_id, RECIPIENTS, T)

        send_log("Workflow completed (local, logs-only)")
        print("Workflow completed locally (DEK + threshold shares). Outputs in output folder", flush=True)
    except Exception as e:
